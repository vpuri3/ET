...
extern "C" void CT_UpdateBoundaries(CCTK_ARGUMENTS, const char *varname)
{
  ...
  if (CCTK_Equals(veryverbose, "yes")) CCTK_VINFO("Updating boundaries of %s...", varname);
...
//      {
        if (CCTK_SyncGroup(cctkGH, varname) < 0)
          CCTK_VERROR("Couldn't sync variable %s", varname); 
//      }
//    } END_COMPONENT_LOOP;
//  } END_MAP_LOOP;
  ...
  if (!CCTK_Equals(boundary_conditions, "none") && CCTK_Equals(varname, "CT_MultiLevel::psi"))
  {
    ...
    if (ierr < 0)
      CCTK_VERROR("Failed to register boundary conditions for %s.", varname);
