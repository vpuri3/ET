...
CCTK_REAL *a0 = cptr.extras[3];
CCTK_REAL *mW = cptr.extras[4];
...
cptr.ct_auxiliary[aindex] = ( -2.0 * pow(cptr.ct_psi[sindex]+a0[sindex], 6.0) * dxK[sindex] + psiXxx + psiYxy + psiZxz) / 3.0;

nauxiliary = 2;
aindex = sindex + nauxiliary * stnc.npoints;
cptr.ct_auxiliary[aindex] = ( -2.0 * pow(cptr.ct_psi[sindex]+a0[sindex], 6.0) * dyK[sindex] + psiXxy + psiYyy + psiZyz) / 3.0;

nauxiliary = 3;
aindex = sindex + nauxiliary * stnc.npoints;
cptr.ct_auxiliary[aindex] = ( -2.0 * pow(cptr.ct_psi[sindex]+a0[sindex], 6.0) * dzK[sindex] + psiXxz + psiYyz + psiZzz) / 3.0;

CCTK_REAL mWxx, mWyy, mWzz;
SecondDerivative(mW, stnc, 0, &tmp, &tmp, &tmp, &mWxx, &mWyy, &mWzz);
nauxiliary = 4;
aindex = sindex + nauxiliary * stnc.npoints;
cptr.ct_auxiliary[aindex] = - mWxx - mWyy - mWzz;
